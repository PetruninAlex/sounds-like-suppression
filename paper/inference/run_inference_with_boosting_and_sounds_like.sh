#!/usr/bin/env bash
# Biased ASR inference (GPU-PB / TurboBias) with a *sounds-like* key-phrases
# list for a single decoder setting.
#
# Self-contained script: calls `python speech_to_text_eval.py` directly
# with all necessary Hydra overrides for boosting.
#
# Takes TWO key-phrases files and concatenates them internally:
#   - ${KEY_PHRASES}  (canonical sweep file from §8/boost_only_files/, word_B)
#   - ${SOUNDS_LIKE}  (sounds-like sweep file from §8/boost_with_sounds_like_files/)
# Each line is `phrase_value`: positive values boost, negative values suppress.
# The sounds-like file carries both the target boost (word_B) and the
# suppressed confusion (soundslike_-W) as separate lines.
#
...
#
# Optional env vars:
#   DATASET, KEY_PHRASES, SOUNDS_LIKE, BOOST_SUFFIX, HYBRID_BEAM_SIZE,
#   AED_BEAM_SIZE, BT_ALPHA, BT_CONTEXT_SCORE, BATCH_SIZE, LONG_DUR_S,
#   LONG_ATT_CTX, MANIFEST, HYBRID_MODEL, CANARY_MODEL,
#   CALCULATE_RTFX, RTFX_WARMUP_STEPS, ...
#
# Output: predictions/${DATASET}/${SETTING}/boost_with_sounds_like_transcriptions/boost_sl${BOOST_SUFFIX}.json
...
if [ -n "${BOOST_SUFFIX}" ] && { [ -z "${KEY_PHRASES:-}" ] || [ -z "${SOUNDS_LIKE:-}" ]; }; then
    echo "[turbobias-sl] ERROR: BOOST_SUFFIX='${BOOST_SUFFIX}' is set but KEY_PHRASES or SOUNDS_LIKE is not." >&2
    echo "[turbobias-sl]        Set both for the correct sweep iteration." >&2
    exit 1
fi
KEY_PHRASES="${KEY_PHRASES:-${DATASET_OUT}/${SETTING}/boost_only_files/boost1.txt}"
SOUNDS_LIKE="${SOUNDS_LIKE:-${DATASET_OUT}/${SETTING}/boost_with_sounds_like_files/boost_with_sounds_like_1.txt}"
...
BOOST_TOKENS_FILE="${BOOST_TOKENS_FILE:-}"
...
# ----------------------------------------------------------- validate & merge
for f in "${KEY_PHRASES}" "${SOUNDS_LIKE}"; do
    if [ ! -f "${f}" ]; then
        echo "[turbobias-sl] ERROR: key-phrases file not found: ${f}" >&2
        echo "[turbobias-sl]        run §8 for SETTING=${SETTING} first, or override" >&2
        echo "[turbobias-sl]        KEY_PHRASES=... and SOUNDS_LIKE=..." >&2
        exit 1
    fi
done

KEY_PHRASES_MERGED="$(mktemp)"
trap 'rm -f "${KEY_PHRASES_MERGED}"' EXIT
cat "${KEY_PHRASES}" "${SOUNDS_LIKE}" > "${KEY_PHRASES_MERGED}"
echo "[turbobias-sl] merged $(wc -l < "${KEY_PHRASES}") canonical + $(wc -l < "${SOUNDS_LIKE}") sounds-like lines -> ${KEY_PHRASES_MERGED}"
...
# Dispatch on SETTING
# ===========================================================================
OUT_SUBDIR="${OUT_SUBDIR:-boost_with_sounds_like_transcriptions}"
OUT_BASE="${DATASET_OUT}/${SETTING}/${OUT_SUBDIR}/boost_sl${BOOST_SUFFIX}"
...
case "${SETTING}" in
    ctc_greedy|ctc_beam|rnnt_greedy|rnnt_beam)
        ...
        case "${SETTING}" in
            ctc_greedy)
                ...
                DECODING_ARGS=(
                    ...
                    ctc_decoding.greedy.boosting_tree.key_phrases_file="${KEY_PHRASES_MERGED}"
                    ...
                    ctc_decoding.greedy.boosting_tree.word_tokens_file="${WORD_TOKENS_FILE}"
                    ctc_decoding.greedy.boosting_tree.boost_tokens_file="${BOOST_TOKENS_FILE}"
            ...
            ctc_beam)
                ...
                DECODING_ARGS=(
                    ...
                    ctc_decoding.beam.boosting_tree.key_phrases_file="${KEY_PHRASES_MERGED}"
                    ...
                    ctc_decoding.beam.boosting_tree.word_tokens_file="${WORD_TOKENS_FILE}"
                    ctc_decoding.beam.boosting_tree.boost_tokens_file="${BOOST_TOKENS_FILE}"
            ...
            rnnt_greedy)
                ...
                DECODING_ARGS=(
                    ...
                    rnnt_decoding.greedy.boosting_tree.key_phrases_file="${KEY_PHRASES_MERGED}"
                    ...
                    rnnt_decoding.greedy.boosting_tree.word_tokens_file="${WORD_TOKENS_FILE}"
                    rnnt_decoding.greedy.boosting_tree.boost_tokens_file="${BOOST_TOKENS_FILE}"
            ...
            rnnt_beam)
                ...
                DECODING_ARGS=(
                    ...
                    rnnt_decoding.beam.boosting_tree.key_phrases_file="${KEY_PHRASES_MERGED}"
                    ...
                    rnnt_decoding.beam.boosting_tree.word_tokens_file="${WORD_TOKENS_FILE}"
                    rnnt_decoding.beam.boosting_tree.boost_tokens_file="${BOOST_TOKENS_FILE}"
                    rnnt_decoding.beam.boosting_tree_alpha="${BT_ALPHA}"
                ) ;;
        esac

        # --- Two-pass decode (short + long) ---
        echo "[turbobias-sl] ${OUT_BASE} (two-pass, decoder=${DEC_TYPE})"
        ...
        # Pass A: short clips
        if [ -s "${SHORT_M}" ]; then
            echo "[turbobias-sl]   pass A (short, batch=${BATCH_SIZE}) -> ${OUT_BASE}_short.json"
            python "${EVAL_PY}" \
            ...
            echo "[turbobias-sl]   pass A skipped (no short clips)"
            : > "${OUT_BASE}_short.json"
        fi

        # Pass B: long clips (batch=1, local attention)
        if [ -s "${LONG_M}" ]; then
            echo "[turbobias-sl]   pass B (long, batch=1, local-attn) -> ${OUT_BASE}_long.json"
            python "${EVAL_PY}" \
                model_path="${HYBRID_MODEL}" \
            ...
            echo "[turbobias-sl]   pass B skipped (no long clips)"
            : > "${OUT_BASE}_long.json"
        fi
        ...
        echo "[turbobias-sl] -> ${OUT_BASE}.json"
        ;;
    ...
    aed_greedy|aed_beam)
        ...
        AED_BASE=(
            ...
            multitask_decoding.beam.boosting_tree.key_phrases_file="${KEY_PHRASES_MERGED}"
            ...
            multitask_decoding.beam.boosting_tree.word_tokens_file="${WORD_TOKENS_FILE}"
            multitask_decoding.beam.boosting_tree.boost_tokens_file="${BOOST_TOKENS_FILE}"
        ...
        echo "[turbobias-sl] ${OUT_BASE} (two-pass AED, beam=${BEAM_SIZE})"
        split_manifest_by_duration "${MANIFEST}" "${LONG_DUR_S}"
        ...
        # Pass A: short clips
        if [ -s "${SHORT_M}" ]; then
            echo "[turbobias-sl]   pass A (short, batch=${AED_BATCH_SIZE}) -> ${OUT_BASE}_short.json"
            python "${EVAL_PY}" \
                dataset_manifest="${SHORT_M}" \
    ...
    *)
        echo "[turbobias-sl] ERROR: unknown SETTING='${SETTING}'" >&2
        echo "[turbobias-sl]        must be one of: ctc_greedy ctc_beam rnnt_greedy rnnt_beam aed_greedy aed_beam" >&2
        exit 1
        ;;
esac
